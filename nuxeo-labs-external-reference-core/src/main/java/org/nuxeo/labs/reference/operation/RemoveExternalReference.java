 ...
 * @author fvadon
 */
@Operation(id=RemoveExternalReference.ID, category=Constants.CAT_DOCUMENT, label="RemoveExternalReference", description="Remove the entries corresponding the tuple formed by ExternalRef and DocumentUID, if only of them is provided, then remove all entries with that item. SHould probably be run as an Admin, not check is done on permissions")
